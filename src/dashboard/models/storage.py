...
from dashboard.ingestion.price_history.service import (
    PriceHistoryIngestionService as PriceHistoryIngestionService,
)
...
class DashboardManager(AnalyticsCommands, BrokerCommands, BusinessStrengthCommands, IngestionCommands, StreamingCommands):
    ...
    def check_new_portfolio_id(self, name: str):
        id, created = self.conn.execute(
            qry.CHECK_NEW_PORTFOLIO_ID,
            [name],
        ).fetchone()
        return id, created
    ...
    def upsert_portfolio(self, name: str, base_ccy: str = "CAD"):
        ...
        self.conn.execute(
            qry.UPSERT_PORTFOLIO_USER,
            [id, name, base_ccy],
        )
        return created

    def _upsert_portfolio_import(
        self, id: int, name: str, created_at: datetime, updated_at: datetime
    ):
        """
    ...
    def list_txns_by_day(self, date_str: str, N: int | None):
        ...
        for fmt in ("%m-%d-%Y", "%m/%d/%Y"):
            ...
            raise AttributeError(
                f"Date {date_str} invalid. Please enter in (MM-DD-YYYY) or (MM/DD/YYYY) format."
            )

        rows = self.conn.execute(
            f"{qry.LIST_TXNS_BY_DAY};",
            [date],
        ).fetchall()
        if not rows:
    ...
    def list_positions_by_asset(self, asset_id: str, N: None | int):
        ...
        """
        rows = self.conn.execute(
            f"{qry.LIST_POSITIONS_BY_ASSET_ID};",
            [asset_id],
        ).fetchall()
        if not rows:
    ...
    def list_positions_by_type(self, asset_type: str, N: int | None):
        ...
        """
        rows = self.conn.execute(
            f"{qry.LIST_POSITIONS_BY_ASSET_TYPE};",
            [asset_type],
        ).fetchall()
        if not rows:
    ...
    def list_positions_by_asset_size(self, asset_size: str, N: int | None):
        ...
        """
        rows = self.conn.execute(
            f"{qry.LIST_POSITIONS_BY_ASSET_SIZE};",
            [asset_size],
        ).fetchall()
        if not rows:
...
class PortfolioManager:
    ...
    def list_positions_by_asset(self, asset_id: str, N: int | None):
        ...
        if not rows:
            raise ValueError(
                f"No positions in portfolio: {self.portfolio_name} with asset:{asset_id}."
            )
    ...
    def list_positions_by_type(self, asset_type: str, N: int | None):
        ...
        rows = self.conn.execute(
            query,
            [asset_type, self.portfolio_id],
        ).fetchall()
        if not rows:
            raise ValueError(
                f"No positions in portfolio: {self.portfolio_name} of type: {asset_type}."
            )
    ...
    def list_positions_by_size(self, asset_size: str, N: int | None):
        ...
        rows = self.conn.execute(
            query,
            [asset_size, self.portfolio_id],
        ).fetchall()
        if not rows:
            raise ValueError(
                f"No positions in portfolio: {self.portfolio_name} of subtype: {asset_size}."
            )
