...
export function PortfolioDetailPage() {
  ...
  const risk = useQuery({ queryKey: ["portfolio-risk", id, benchmark, range], queryFn: () => api.portfolioRisk(id, { benchmark: benchmark || undefined, lookback: range }), enabled: portfolioReady && (resolvedTab === "overview" || resolvedTab === "risk") });
  const fundamentals = useQuery({ queryKey: ["portfolio-fundamentals", id, 5], queryFn: () => api.portfolioFundamentals(id, 5), enabled: portfolioReady && (resolvedTab === "overview" || resolvedTab === "fundamentals") });
        ...
        <PieChart accessibilityLayer={false}>
          <defs>
            ...
            {groups.map((group, index) => {
              ...
              return <linearGradient key={`${group.label}-face`} id={`${chartId}-slice-${index}`} x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" dy="-10" className="allocation-center-value">{money(totalValue, currency)}</text>
          <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" dy="16" className="allocation-center-label">Total</text>
