...
from dashboard.services.importer import TxnImporterCSV, TxnImporterManual

TMP_FOLDER = "tests/tmp/"
TEST_IMPORTER_DB = "test_importer.db"

@pytest.fixture
def test_manager(tmp_folder: str = TMP_FOLDER, test_db: str = TEST_IMPORTER_DB):
    """
...
def test_import_one_port_batch(test_manager: PortfolioManager):
    ...
    """
    importer = TxnImporterCSV(test_manager)

    csv_path = TMP_FOLDER + "test_import_one_batch.csv"
    ...
    assert p_aff.portfolio_name == "test"
...
def test_import_mul_port_batch(test_manager: PortfolioManager):
    ...
    csv_path = TMP_FOLDER + "test_import_mul_port_batch.csv"
    csv_path.write_text("\n".join(["portfolio_name,time_stamp,txn_type,asset_id,qty,price,ccy,cash_amt,fee_amt",
    ...
    # ImportData assertions
    assert import_data.inserted_rows == 3
    ...
    for p_aff in portfolios_aff:
        assert p_aff.batch_id == portfolios_aff.batch_id
        assert p_aff.portfolio_name in ["test 1", "test 2"]
        if p_aff.created:
            assert p_aff.portfolio_name != "test 1"

    # db: txn count and seq. id assertions
    n = test_manager.conn.execute("SELECT COUNT(*) FROM txn").fetchone()[0]
    assert n == import_data.inserted_rows+2
    ...
    amt_tuple = test_manager.conn.execute(
        "SELECT cash_amt, fee_amt FROM txn WHERE portfolio_name = ? AND asset_id = ?", ["test 1", "AVUV"],
        ).fetchone()[0]
...
def test_manual_txn_upd(test_manager: PortfolioManager):
    ...
    """
    importer = TxnImporterManual(test_manager)
    batch_id = importer.batch_id
    importer.create_portfolio = False 
    importer.create_portfolio_name = None

    importer.txn = Txn(
        portfolio_id=1,
        time_stamp="2026-01-05 13:15:00",
    ...
    # db: txn count and seq. id assertions
    n = test_manager.conn.execute("SELECT COUNT(*) FROM txn").fetchone()[0]
    assert n == import_data.inserted_rows+6
...
def test_manual_txn_create(test_manager: PortfolioManager):
    ...
    importer = TxnImporterManual(test_manager)
    batch_id = importer.batch_id
    importer.create_portfolio = True 
    importer.create_portfolio_name = "test 3"
    p_id = 3

    importer.txn = Txn(
        portfolio_id=p_id,
        time_stamp="2026-01-06 21:12:21",
